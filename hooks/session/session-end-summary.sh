...
END_REASON=$(printf '%s' "$INPUT" | jq -r '.reason // .matcher // "unknown"')
...
if [[ -n "$TRANSCRIPT" && -f "$TRANSCRIPT" ]]; then
  ...
  if [[ -n "$START_TS" ]]; then
    ...
    elif date -j -f "%Y-%m-%dT%H:%M:%S" "${START_TS%.*}" +%s >/dev/null 2>&1; then
      START_EPOCH=$(date -j -f "%Y-%m-%dT%H:%M:%S" "${START_TS%.*}" +%s)
    fi
